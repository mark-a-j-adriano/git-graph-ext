# GitGraph V2 for Visual Studio Code

View a Git Graph of your repository, and easily perform Git actions from the graph. Configurable to look the way you want.

## Enhancement Summary

GitGraph V2 focuses on making the extension more intentional about when it does repository work, so large workspaces do not pay the full startup cost before the user asks for the graph.

### Problem Statement

- The extension previously activated too eagerly.
- Startup could trigger broad repository discovery, watcher startup, config checks, and submodule scans even when the workspace already had enough known state to render the graph later.
- In large repositories and monorepos, that creates avoidable CPU, I/O, and memory pressure during editor startup.

### What This Solves

- Reduces unnecessary background work at startup.
- Defers expensive repository discovery until the user or UI explicitly asks for it.
- Keeps known repositories fresh through targeted refresh flows instead of broad rescans.
- Narrows extension activation so VS Code does not eagerly load the extension for every window.

### New Features In This Enhancement

- Lazy startup repository discovery when known repositories already exist.
- Deferred startup watcher creation for workspace-wide folder scans.
- Explicit `discoverWorkspaceRepos` and `refreshKnownRepos` flows for on-demand refresh behavior.
- UI-triggered refresh from the Git Graph webview `loadRepos` and `rescanForRepos` paths.
- Startup deferral for known-repo config checks and submodule scans when lazy startup is active.
- Manifest activation cleanup so the extension no longer relies on wildcard activation.

The full written summary is available in [ENHANCEMENT_SUMMARY.md](./ENHANCEMENT_SUMMARY.md). Distribution terms for packaged builds are in [LICENSE-DISTRIBUTION.md](./LICENSE-DISTRIBUTION.md).

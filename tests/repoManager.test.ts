...
beforeAll(() => {
  ...
  jest
    ...
    .mockImplementation(
      <T>(onItem: (item: T) => Promise<boolean>, onChanges: () => void) => {
        const realBufferedQueue = jest.requireActual(
...
describe("RepoManager", () => {
  ...
  describe("onDidChangeWorkspaceFolders", () => {
    it("Should add repositories contained within an added workspace folder", async () => {
      ...
      vscode.workspace.onDidChangeWorkspaceFolders.mockImplementationOnce(
        (listener) => {
      ...
      repoManager.onDidChangeRepos((event) =>
        onDidChangeReposEvents.push(event),
      );
      ...
      expect(repoManager.getRepos()).toStrictEqual({
        "/path/to/workspace-folder1": mockRepoState({
      ...
      expect(spyOnLog).toHaveBeenCalledWith(
        "Added new repo: /path/to/workspace-folder1",
      );
      expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
    ...
    it("Should remove repositories contained within a removed workspace folder", async () => {
      ...
      expect(spyOnLog).toHaveBeenCalledWith(
        "Removed repo: /path/to/workspace-folder2",
      );
    ...
    it("Should update all repositories workspaceFolderIndex's when workspace folders have been reordered", async () => {
      ...
      expect(repoManager.getRepos()).toStrictEqual({
        ...
        "/path/to/workspace-folder2": mockRepoState({
          workspaceFolderIndex: 1,
        }),
        "/path/to/workspace-folder3": mockRepoState({
    ...
    it("Should not trigger a workspace search if the value hasn't increased", async () => {
      ...
      onDidChangeConfiguration.emit({
        affectsConfiguration: (section) =>
          section === "git-graph.maxDepthOfRepoSearch",
      });
  ...
  describe("startupTasks", () => {
    it("Should run startup tasks without discovering additional repos when known repos already exist", async () => {
      // Setup
      ...
      expect(spyOnSaveRepos).toHaveBeenCalledTimes(3);
      expect(spyOnSaveRepos).toHaveBeenCalledWith({
        "/path/to/workspace-folder1/repo1": mockRepoState({
          workspaceFolderIndex: 0,
        }),
  ...
  describe("removeReposNotInWorkspace", () => {
    ...
    it("Should remove all repositories when no workspace folders exist", async () => {
      ...
      expect(repoManager.getRepos()).toStrictEqual({});
      expect(spyOnLog).toHaveBeenCalledWith(
        "Removed repo: /path/to/workspace-folder1",
      );
      expect(spyOnLog).toHaveBeenCalledWith(
        "Removed repo: /path/to/workspace-folder2",
      );
    ...
    it("Shouldn't remove repositories that are within a workspace folder", async () => {
      ...
      await waitForExpect(() => {
        ...
        expect(spyOnRepoRoot).toHaveBeenCalledWith(
          "/path/to/workspace-folder1/repo",
        );
        expect(spyOnRepoRoot).toHaveBeenCalledWith(
          "/path/to/workspace-folder1",
  ...
  describe("registerRepo", () => {
    it("Should register a new repository", async () => {
      ...
      const result = await repoManager.registerRepo(
        "/path/to/workspace-folder2/repo",
        false,
      );
      ...
      expect(spyOnLog).toHaveBeenCalledWith(
        "Added new repo: /path/to/workspace-folder2/repo",
      );
      expect(onDidChangeReposEvents).toStrictEqual([
        {
          repos: {
            "/path/to/workspace-folder2/repo": mockRepoState({
              workspaceFolderIndex: 1,
            }),
          },
          numRepos: 1,
      ...
      expect(
        utils.getPathFromStr(spyOnReadFile.mock.calls[0][0]),
      ).toStrictEqual(
    ...
    it("Should register a new repository (loading it upon registration)", async () => {
      ...
      expect(spyOnLog).toHaveBeenCalledWith(
        "Added new repo: /path/to/workspace-folder1/repo",
      );
      expect(onDidChangeReposEvents).toStrictEqual([
        {
          repos: {
    ...
    it("Should register a new repository (should only update the workspaceFolderIndex for the repository that was added)", async () => {
      ...
      repoManager["repos"]["/path/to/workspace-folder1"].workspaceFolderIndex =
        null;
    ...
    it("Should return an error message when the path being registered is not a Git repository", async () => {
      ...
      expect(result).toStrictEqual({
        root: null,
        error:
          'The folder "/path/to/workspace-folder1" is not a Git repository.',
      });
  ...
  describe("getRepos", () => {
    it("Should get the set of repositories", async () => {
      ...
      expect(result).toStrictEqual({
        ...
        "/path/to/workspace-folder1/repo2": mockRepoState({
          workspaceFolderIndex: 0,
        }),
      });
  ...
  describe("getRepoContainingFile", () => {
    it("Should return the path of the repository that contains the file", async () => {
      ...
      const result = repoManager.getRepoContainingFile(
        "/path/to/workspace-folder1/repo2/file.txt",
      );
    ...
    it("Should return the path of the closest repository that contains the file", async () => {
      ...
      const repoManager = await constructRepoManagerAndWaitUntilStarted(
        ...
        [
          "/path/to/workspace-folder1/repo1",
          "/path/to/workspace-folder1/repo1/subrepo",
        ],
      );
  ...
  describe("getKnownRepo", () => {
    it("Should return the path of the repository matching the specified path", async () => {
      ...
      // Run
      const result = await repoManager.getKnownRepo(
        "/path/to/workspace-folder1/repo1",
      );
    ...
    it("Should return the path of the repository matching the specified path (resolving symbolic path components)", async () => {
      ...
      const repoManager = await constructRepoManagerAndWaitUntilStarted(
        ["/path/to/workspace-folder1"],
        [
          "/path/to/workspace-folder1/sym-repo1",
          "/path/to/workspace-folder1/sym-repo2",
        ],
      );
  ...
  describe("isKnownRepo", () => {
    ...
    it("Should return FALSE when the repository is not known", async () => {
      ...
      // Run
      const result = repoManager.isKnownRepo(
        "/path/to/workspace-folder1/other-repo",
      );
  ...
  describe("checkReposExist", () => {
    it("Should remove any repositories that no longer exist", async () => {
      ...
      expect(result).toBe(true);
      expect(spyOnLog).toHaveBeenCalledWith(
        "Removed repo: /path/to/workspace-folder1/repo2",
      );
      expect(onDidChangeReposEvents).toStrictEqual([
        {
          repos: {
    ...
    it("Should transfer any repository state if the path to the repository changes", async () => {
      ...
      expect(repoManager.getRepos()).toStrictEqual({
        ...
        "/path/to/workspace-folder2/repo2": mockRepoState({
          workspaceFolderIndex: 1,
        }),
      });
      expect(spyOnSaveRepos).toHaveBeenCalledWith({
    ...
    it("Should return gracefully when an exception occurs", async () => {
      ...
      expect(result).toBe(true);
      expect(spyOnSaveRepos).toHaveBeenCalledWith({
        "/path/to/workspace-folder2/repo1": mockRepoState({
          workspaceFolderIndex: 1,
        }),
        "/path/to/workspace-folder2/repo2": mockRepoState({
          workspaceFolderIndex: 1,
        }),
        "/path/to/workspace-folder1/repo3": mockRepoState({
          workspaceFolderIndex: 0,
        }),
      });
      expect(onDidChangeReposEvents).toStrictEqual([
  ...
  describe("searchWorkspaceForRepos", () => {
    ...
    it("Should add repositories that are detected in subfolders of the workspace", async () => {
      ...
      expect(spyOnReaddir).toHaveBeenCalledWith(
        "/path/to/workspace-folder1",
        expect.anything(),
      );
    ...
    it("Should halt recursion when fs.readdir returns an error", async () => {
      ...
      await waitForExpect(() =>
        expect(spyOnLog).toHaveBeenCalledWith(
          "Completed searching workspace for new repos",
        ),
      );
  ...
  describe("discoverWorkspaceRepos", () => {
    it("Should expose an explicit workspace discovery entry point", async () => {
      // Setup
      mockDirectoryThatsNotRepository();
      const repoManager = await constructRepoManagerAndWaitUntilStarted(
        ["/path/to/workspace-folder1"],
      ...
      // Run
      const result = await repoManager.discoverWorkspaceRepos();
  ...
  describe("refreshKnownRepos", () => {
    it("Should refresh known repositories without searching the workspace for new repos", async () => {
      // Setup
      mockDirectoryThatsNotRepository();
      mockRepositoryWithNoSubmodules();
      const repoManager = await constructRepoManagerAndWaitUntilStarted(
      ...
      mockRepositoryWithSubmodules([
        "/path/to/workspace-folder1/repo/submodule",
      ]);
      mockRepositoryWithNoSubmodules();

      const searchWorkspaceForReposSpy = jest.spyOn(
      ...
      // Run
      await repoManager.refreshKnownRepos();

      // Assert
      expect(searchWorkspaceForReposSpy).not.toHaveBeenCalled();
      expect(repoManager.getRepos()).toStrictEqual({
        "/path/to/workspace-folder1/repo": mockRepoState({
          workspaceFolderIndex: 0,
        }),
        "/path/to/workspace-folder1/repo/submodule": mockRepoState({
          workspaceFolderIndex: 0,
        }),
      });
      expect(onDidChangeReposEvents).toStrictEqual([
  ...
  describe("checkReposForNewSubmodules", () => {
    it("Should add any new submodules", async () => {
      ...
      expect(repoManager.getRepos()).toStrictEqual({
        ...
        "/path/to/workspace-folder1/repo/submodule1": mockRepoState({
          workspaceFolderIndex: 0,
        }),
        "/path/to/workspace-folder1/repo/submodule2": mockRepoState({
          workspaceFolderIndex: 0,
        }),
      });
  ...
  describe("onWatcherCreate", () => {
    ...
    it("Should add a repository when a repository is added", async () => {
      ...
      // Assert
      expect(spyOnEnqueue).toHaveBeenCalledWith(
        "/path/to/workspace-folder1/repo",
      );
      await waitForExpect(() => {
    ...
    it("Should not proceed to add repositories when the directory added is within .git", () => {
      // Run
      emitOnDidCreate(
        vscode.Uri.file("/path/to/workspace-folder1/repo/.git/folder"),
      );
  ...
  describe("onWatcherChange", () => {
    ...
    it("Should remove a repository when a repository is deleted", async () => {
      ...
      await waitForExpect(() => {
        ...
        expect(spyOnLog).toHaveBeenCalledWith(
          "Removed repo: /path/to/workspace-folder1/repo",
        );
        expect(repoManager.getRepos()).toStrictEqual({});
        expect(onDidChangeReposEvents).toStrictEqual([
          {
  ...
  describe("onWatcherDelete", () => {
    it("Should delete repositories within a deleted directory", async () => {
      ...
      // Assert
      expect(spyOnLog).toHaveBeenCalledWith(
        "Removed repo: /path/to/workspace-folder1/dir/repo1",
      );
      expect(spyOnLog).toHaveBeenCalledWith(
        "Removed repo: /path/to/workspace-folder1/dir/repo2",
      );
      expect(repoManager.getRepos()).toStrictEqual({
        "/path/to/workspace-folder1/repo3": mockRepoState({
          workspaceFolderIndex: 0,
    ...
    it("Should delete repositories within a deleted repository", async () => {
      ...
      // Assert
      expect(spyOnLog).toHaveBeenCalledWith(
        "Removed repo: /path/to/workspace-folder1/repo1",
      );
      expect(spyOnLog).toHaveBeenCalledWith(
    ...
    it("Should not proceed to remove repositories if the deleted URI is within .git", async () => {
      ...
      // Run
      emitOnDidDelete(
        vscode.Uri.file("/path/to/workspace-folder1/dir/repo1/.git/folder"),
      );
      ...
      expect(repoManager.getRepos()).toStrictEqual({
        "/path/to/workspace-folder1/dir/repo1": mockRepoState({
          workspaceFolderIndex: 0,
        }),
  ...
  describe("checkRepoForNewConfig", () => {
    describe("apply config", () => {
      ...
      describe("name", () => {
        it(
          "Should import name correctly",
          testApplyField("name", "Name", "name", "Name"),
        );
      });
    ...
    describe("validation", () => {
      const testValidationOfField =
        (fileKey: string, fileValue: any) => async () => {
          // Setup
          mockDirectoryThatsNotRepository();
          const repoManager = await constructRepoManagerAndWaitUntilStarted(
            ["/path/to/workspace-folder1"],
  ...
  describe("onConfigWatcherCreateOrChange", () => {
    it("Should import the repository configuration when the file is created", async () => {
      ...
      const emitOnDidCreate = (<jest.Mock<any, any>>(
        repoManager["configWatcher"].onDidCreate
      )).mock.calls[0][0];
      const spyOnBufferedQueueEnqueue = jest.spyOn(
        repoManager["checkRepoConfigQueue"],
        "enqueue",
      );
      mockFsReadFileOnce(null, {
        showTags: true,
      ...
      expect(spyOnBufferedQueueEnqueue).toHaveBeenCalledWith(
        "/path/to/workspace-folder1/repo1",
      );
    });
    ...
    it("Should import the repository configuration when the file is changed", async () => {
      ...
      repoManager["repos"]["/path/to/workspace-folder1/repo1"].name =
        "Old Name";
      const emitOnDidChange = (<jest.Mock<any, any>>(
        repoManager["configWatcher"].onDidChange
      )).mock.calls[0][0];
      const spyOnBufferedQueueEnqueue = jest.spyOn(
        repoManager["checkRepoConfigQueue"],
        "enqueue",
    ...
    it('Shouldn\'t import the repository configuration when user responds "No"', async () => {
      ...
      // Assert
      await waitForExpect(() =>
        expect(spyOnIsKnownRepo).toHaveBeenCalledWith(
          "/path/to/workspace-folder1/repo1",
        ),
      );
      const repoState =
        repoManager.getRepos()["/path/to/workspace-folder1/repo1"];
    ...
    it("Shouldn't import the repository configuration when it is not for a known repository", async () => {
      ...
      emitOnDidChange(
        vscode.Uri.file(
          "/path/to/workspace-folder1/repo2/.vscode/vscode-git-graph.json",
        ),
      );
  ...
  describe("exportRepoConfig", () => {
    it("Should export the repository configuration", async () => {
      ...
      // Run
      const result = await repoManager.exportRepoConfig(
        "/path/to/workspace-folder1/repo1",
      );
    ...
    const testExportField =
      ...
      async () => {
        ...
        repoManager.getRepos()["/path/to/workspace-folder1/repo1"][stateKey] =
          stateValue;
    ...
    describe("name", () => {
      it(
        "Should export name correctly",
        testExportField("name", "Name", "name", "Name"),
      );
    });
...
function constructRepoManager(
  ...
  ignoreRepos: string[] = [],
  lazyStartupRepoDiscovery: boolean = false,
) {
  ...
  vscode.workspace.workspaceFolders = workspaceFolders
    ? workspaceFolders.map((path, index) => ({
...
function waitForRepoManagerToStart() {
  return waitForExpect(() => {
    expect(
      spyOnLog.mock.calls.some(
        (call) =>
          call[0] === "Completed searching workspace for new repos" ||
          call[0] ===
            "Skipped searching workspace for new repos during startup",
      ),
    ).toBe(true);

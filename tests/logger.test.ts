...
describe("Logger", () => {
  ...
  it("Should log an error to the Output Channel", () => {
    ...
    // Assert
    expect(outputChannel.appendLine).toHaveBeenCalledWith(
      "[2020-04-22 12:40:58.010] ERROR: Test",
    );
  });

  it("Should record timed operations without logging fast ones", () => {
    // Run
    logger.logDuration("workspace repo scan", 125, 250);

    // Assert
    expect(outputChannel.appendLine).not.toHaveBeenCalled();
    expect(logger.getDiagnostics()).toMatchObject({
      timedOperationCount: 1,
      slowOperationCount: 0,
    });
  });

  it("Should log slow operations and include them in diagnostics", () => {
    // Setup
    date.setCurrentTime(1587559258.25);

    // Run
    logger.logDuration("git fetch", 750, 500, "repo-name");

    // Assert
    expect(outputChannel.appendLine).toHaveBeenCalledWith(
      "[2020-04-22 12:40:58.250] SLOW: git fetch completed in 750ms (repo-name)",
    );
    expect(logger.getDiagnostics()).toMatchObject({
      timedOperationCount: 1,

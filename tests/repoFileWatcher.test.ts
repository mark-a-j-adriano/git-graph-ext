...
describe("RepoFileWatcher", () => {
  ...
  it("Should start and receive file events", () => {
    ...
    const onDidCreate = (<jest.Mock<any, any>>(
      repoFileWatcher["fsWatcher"]!.onDidCreate
    )).mock.calls[0][0];
    const onDidChange = (<jest.Mock<any, any>>(
      repoFileWatcher["fsWatcher"]!.onDidChange
    )).mock.calls[0][0];
    const onDidDelete = (<jest.Mock<any, any>>(
      repoFileWatcher["fsWatcher"]!.onDidDelete
    )).mock.calls[0][0];
  ...
  it("Should stop a previous active File System Watcher before creating a new one", () => {
    ...
    const onDidCreate = (<jest.Mock<any, any>>watcher.onDidCreate).mock
      .calls[0][0];
  ...
  it("Should not recreate the File System Watcher when starting the same repo twice", () => {
    // Run
    repoFileWatcher.start("/path/to/repo");
    const watcher = repoFileWatcher["fsWatcher"]!;
    repoFileWatcher.start("/path/to/repo");
    ...
    expect(<jest.Mock<any, any>>watcher.dispose).toHaveBeenCalledTimes(0);
  });
  ...
  it("Should expose watcher diagnostics", () => {
    // Run
    repoFileWatcher.start("/path/to/repo");
    const diagnostics = repoFileWatcher.getDiagnostics();

...
describe("AvatarManager", () => {
  ...
  describe("clearCache", () => {
    ...
    it("Should clear the cache of avatars", async () => {
      ...
      avatarManager["remoteSourceCache"] = {
        "test-repo": { type: "gravatar" },
      };
      avatarManager["interval"] = setInterval(() => undefined, 10000);
      ...
      expect(avatarManager["queue"]["queue"]).toStrictEqual([]);
      expect(avatarManager["remoteSourceCache"]).toStrictEqual({});
      expect(avatarManager["interval"]).toBeNull();

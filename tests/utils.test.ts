...
describe("realpath", () => {
  it("Should return the normalised canonical absolute path", async () => {
    ...
    // Assert
    expect(path).toBe("/a/b");
    expect(mockedFileSystemModule.realpath).toBeCalledWith(
      "\\a\\b",
      expect.anything(),
    );
  ...
  it("Should return the normalised canonical absolute path (using the native version realpath)", async () => {
    ...
    expect(mockedFileSystemModule.realpath.native).toBeCalledWith(
      "\\a\\b",
      expect.anything(),
    );
  });
...
describe("resolveToSymbolicPath", () => {
  ...
  it("Should return the original path if it is within a vscode workspace folder", async () => {
    ...
    // Run
    const result = await resolveToSymbolicPath(
      "/path/to/workspace-folder1/subfolder/file.txt",
    );
  ...
  it("Should return the original path if the vscode workspace folder resolves to be contained within it, when it was unable to find the path correspondence", async () => {
    ...
    mockedFileSystemModule.realpath.mockImplementation(
      (
        ...
        callback(
          null,
          path === "/symbolic-folder/path/to/dir"
            ? path.replace("symbolic", "workspace")
            : path,
        );
      },
    );
...
describe("getExtensionVersion", () => {
  ...
  it("Should reject if unable to read package.json file", async () => {
    ...
    // Run
    await getExtensionVersion(vscode.mocks.extensionContext).catch(
      () => (rejected = true),
    );
    ...
    const repoPaths = getSortedRepositoryPaths(
      {
        ...
        "/path/to/another/A": mockRepoState({
          name: null,
          workspaceFolderIndex: 7,
        }),
...
describe("openFile", () => {
  ...
  it("Should open a renamed file in vscode", async () => {
    ...
    vscode.commands.executeCommand.mockResolvedValueOnce(null);
    const spyOnGetNewPathOfRenamedFile = jest.spyOn(
      dataSource,
      "getNewPathOfRenamedFile",
    );
  ...
  it("Should return an error message if the file doesn't exist in the repository", async () => {
    ...
    // Assert
    expect(result).toBe(
      "The file deleted.txt doesn't currently exist in this repository.",
    );
  ...
  it("Should return an error message if the file doesn't exist in the repository, and it was renamed", async () => {
    ...
    // Assert
    expect(result).toBe(
      "The file renamed-old.txt doesn't currently exist in this repository.",
    );
    expect(mockedFileSystemModule.access).toHaveBeenNthCalledWith(
      1,
      path.join("/path/to/repo", "renamed-old.txt"),
...
describe("viewDiff", () => {
  ...
  it("Should load the vscode diff view (between commit and uncommitted changes, file modified)", async () => {
    ...
    expect(getPathFromUri(rightUri)).toBe(
      "/path/to/repo/subfolder/modified.txt",
    );
...
describe("viewDiffWithWorkingFile", () => {
  ...
  it("Should load the vscode diff view (renamed file)", async () => {
    ...
    spyOnGetNewPathOfRenamedFile.mockResolvedValueOnce(
      "subfolder/renamed-new.txt",
    );
    ...
    expect(getPathFromUri(rightUri)).toBe(
      "/path/to/repo/subfolder/renamed-new.txt",
    );
...
describe("evalPromises", () => {
  ...
  it("Should evaluate promises in parallel (one item in array that rejects)", async () => {
    ...
    // Run
    await evalPromises([1], 2, (x) => Promise.reject(x * 2)).catch(
      () => (rejected = true),
    );
  ...
  it("Should evaluate promises in parallel", async () => {
    // Run
    const result = await evalPromises([1, 2, 3, 4], 2, (x) =>
      Promise.resolve(x * 2),
    );
...
describe("resolveSpawnOutput", () => {
  it("Should resolve child process promise only once (error event first)", async () => {
    ...
    // Run
    const result = await resolveSpawnOutput(
      cp.spawn("/path/to/git", ["arg0", "arg1"]),
    );
...
describe("findGit", () => {
  ...
  it("Should use the last known Git executable path if it still exists", async () => {
    // Setup
    jest
      .spyOn(extensionState, "getLastKnownGitPath")
  ...
  it("Should use the users git.path if the last known Git executable path no longer exists", async () => {
    ...
    jest
      .spyOn(extensionState, "getLastKnownGitPath")
      .mockReturnValueOnce("/path/to/not-git");
  ...
  describe("process.platform === 'win32'", () => {
    ...
    it("Should find Git in ProgramW6432", async () => {
      ...
      // Assert
      expect(getPathFromStr(result.path)).toBe(
        "c:/path/to/ProgramW6432/Git/cmd/git.exe",
      );
    ...
    it("Should find Git in ProgramFiles(x86)", async () => {
      ...
      // Assert
      expect(getPathFromStr(result.path)).toBe(
        "c:/path/to/ProgramFilesX86/Git/cmd/git.exe",
      );
      expect(result.version).toBe("1.2.3");
    });
    ...
    it("Should find Git in ProgramFiles", async () => {
      ...
      // Assert
      expect(getPathFromStr(result.path)).toBe(
        "c:/path/to/ProgramFiles/Git/cmd/git.exe",
      );
      expect(result.version).toBe("1.2.3");
    });
    ...
    it("Should find Git in LocalAppData", async () => {
      ...
      // Assert
      expect(getPathFromStr(result.path)).toBe(
        "c:/path/to/LocalAppData/Programs/Git/cmd/git.exe",
      );
      expect(result.version).toBe("1.2.3");
    });
    ...
    it("Should find Git in PATH (isFile)", async () => {
      ...
      mockedFileSystemModule.stat.mockImplementation(
        (
          ...
          callback(null, {
            isFile: () =>
              getPathFromStr(statPath as string) ===
              "c:/path/to/git-dir/git.exe",
...
describe("getGitExecutableFromPaths", () => {
  it("Should return the git version information from the first valid path (all valid)", async () => {
    ...
    // Run
    const result = await getGitExecutableFromPaths([
      "/path/to/first/git",
      "/path/to/second/git",
    ]);
  ...
  it("Should reject when none of the provided paths are valid Git executables", async () => {
    ...
    // Run
    await getGitExecutableFromPaths([
      "/path/to/first/git",
      "/path/to/second/git",
    ]).catch(() => (rejected = true));
...
function expectedValueGitGraphUri(
  filePath: string,
  commit: string,
  repo: string,
  exists: boolean,

...
describe("CommandManager", () => {
  ...
  describe("git-graph.diagnostics", () => {
    let spyOnCopyToClipboard: jest.SpyInstance,
      ...
      spyOnLoggerGetDiagnostics: jest.SpyInstance,
      spyOnRepoManagerGetDiagnostics: jest.SpyInstance,
      spyOnGitGraphViewGetDiagnostics: jest.SpyInstance;
    beforeAll(() => {
      ...
      spyOnLoggerGetDiagnostics = jest.spyOn(logger, "getDiagnostics");
      spyOnRepoManagerGetDiagnostics = jest.spyOn(
        repoManager,
        "getDiagnostics",
      );
      spyOnGitGraphViewGetDiagnostics = jest.spyOn(
        GitGraphView,
        "getDiagnostics",
      );
    });

    it("Should display diagnostics information, and copy it to the clipboard", async () => {
      // Setup
      spyOnGetExtensionVersion.mockResolvedValueOnce("1.30.15");
      ...
      spyOnLoggerGetDiagnostics.mockReturnValueOnce({
        uptimeSeconds: 12,
        loggedLineCount: 50,
        errorCount: 2,
        spawnedGitCommandCount: 8,
      });
      spyOnRepoManagerGetDiagnostics.mockReturnValueOnce({
        knownRepoCount: 3,
        ignoredRepoCount: 1,
        workspaceFolderWatcherCount: 2,
        watchedFolders: ["/path/to/workspace-a", "/path/to/workspace-b"],
        maxDepthOfRepoSearch: 1,
        onWatcherCreateQueue: {
          size: 0,
          processing: false,
          hasPendingTimer: false,
        },
        onWatcherChangeQueue: {
          size: 1,
          processing: false,
          hasPendingTimer: true,
        },
        checkRepoConfigQueue: {
          size: 0,
          processing: true,
          hasPendingTimer: false,
        },
      });
      spyOnGitGraphViewGetDiagnostics.mockReturnValueOnce({
        active: true,
        visible: true,
        loaded: true,
        currentRepo: "/path/to/repo",
        repoFileWatcher: {
          active: true,
          repo: "/path/to/repo",
          muted: false,
      ...
      // Run
      vscode.commands.executeCommand("git-graph.diagnostics");

      // Assert
      await waitForExpect(() => {
        const expectedInformation =
          "Git Graph: 1.30.15\nVisual Studio Code: 1.51.0\nOS: X Y Z\nGit: 2.25.0\n\nLogger:\n- Uptime (s): 12\n- Logged lines: 50\n- Errors: 2\n- Spawned Git commands: 8\n\nRepo Manager:\n- Known repos: 3\n- Ignored repos: 1\n- Workspace folder watchers: 2\n- Watched folders: /path/to/workspace-a, /path/to/workspace-b\n- Max repo search depth: 1\n- Create queue: size=0, processing=false, pendingTimer=false\n- Change queue: size=1, processing=false, pendingTimer=true\n- Config queue: size=0, processing=true, pendingTimer=false\n\nGit Graph View:\n- Active: true\n- Visible: true\n- Loaded: true\n- Current repo: /path/to/repo\n- Repo watcher: active=true, muted=false, pendingRefresh=true, resumeAfterMs=250, repo=/path/to/repo";
        expect(spyOnLog).toHaveBeenCalledWith(
          "Command Invoked: git-graph.diagnostics",
        );
        ...
        expect(spyOnCopyToClipboard).toHaveBeenCalledWith(expectedInformation);
      });
    });

    it("Should display an error message when diagnostics couldn't be retrieved", async () => {
      // Setup
  ...
  describe("git-graph:codiconsSupported", () => {
    ...
    it("Should log an error message when an exception is thrown", async () => {
      ...
      commandManager.dispose();
      const spyOnExecuteCommand = jest.spyOn(vscode.commands, "executeCommand");
      const spyOnDoesVersionMeetRequirement = jest.spyOn(
        utils,
  ...
  describe("git-graph.view", () => {
    it("Should open the Git Graph View", async () => {
      ...
      await waitForExpect(() => {
        expect(spyOnLog).toHaveBeenCalledWith(
          "Command Invoked: git-graph.view",
        );
  ...
  describe("git-graph.addGitRepository", () => {
    ...
    it("Should register the selected repository", async () => {
      ...
      await waitForExpect(() => {
        expect(spyOnLog).toHaveBeenCalledWith(
          "Command Invoked: git-graph.addGitRepository",
        );
        ...
        expect(spyOnIsPathInWorkspace).toHaveBeenCalledWith(
          "/path/to/workspace-folder/repo",
        );
        expect(spyOnRegisterRepo).toHaveBeenCalledWith(
          "/path/to/workspace-folder/repo",
          false,
        );
        expect(vscode.window.showInformationMessage).toHaveBeenCalledWith(
    ...
    it("Should display the error message returned when registering the selected repository", async () => {
      ...
      spyOnRegisterRepo.mockResolvedValueOnce({
        root: null,
        error:
          'The folder "/path/to/workspace-folder/repo" is not a Git repository.',
      });
      vscode.window.showErrorMessage.mockResolvedValueOnce(null);
    ...
    it("Should display an error message when the selected repository is not within the workspace", async () => {
      ...
      await waitForExpect(() => {
        ...
        expect(spyOnIsPathInWorkspace).toHaveBeenCalledWith(
          "/path/to/non-workspace-folder/repo",
        );
        expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
  ...
  describe("git-graph.removeGitRepository", () => {
    ...
    it("Should ignore the selected repository", async () => {
      // Setup
      const repos = {
        "/path/to/repo2": mockRepoState({
          name: "Custom Name",
          workspaceFolderIndex: 1,
        }),
        "/path/to/repo1": mockRepoState({
          name: null,
      ...
      vscode.window.showInformationMessage.mockResolvedValueOnce(null);
      const spyOnGetSortedRepositoryPaths = jest.spyOn(
        utils,
      ...
      await waitForExpect(() => {
        expect(spyOnLog).toHaveBeenCalledWith(
          "Command Invoked: git-graph.removeGitRepository",
        );
  ...
  describe("git-graph.clearAvatarCache", () => {
    ...
    it("Should clear the avatar cache, and display a success message", async () => {
      ...
      await waitForExpect(() => {
        expect(spyOnLog).toHaveBeenCalledWith(
          "Command Invoked: git-graph.clearAvatarCache",
        );
  ...
  describe("git-graph.fetch", () => {
    ...
    it("Should display a quick pick to select a repository to open in the Git Graph View (with last active repository first)", async () => {
      ...
      await waitForExpect(() => {
        expect(spyOnLog).toHaveBeenCalledWith(
          "Command Invoked: git-graph.fetch",
        );
        expect(spyOnGetSortedRepositoryPaths).toHaveBeenCalledWith(
          repos,
          RepoDropdownOrder.WorkspaceFullPath,
  ...
  describe("git-graph.endSpecificWorkspaceCodeReview", () => {
    it("Should end the selected code review", async () => {
      ...
      vscode.window.showQuickPick.mockImplementationOnce(
        (items: Promise<any[]>, _: any) => items.then((items) => items[0]),
      );
  ...
  describe("git-graph.resumeWorkspaceCodeReview", () => {
    ...
    it("Should load the selected code review in the Git Graph View (commit comparison)", async () => {
      ...
      spyOnGetCommitSubject.mockImplementationOnce(
        (_: string, hash: string) => "subject-" + hash,
      );
      spyOnGetCommitSubject.mockImplementationOnce(
        (_: string, hash: string) => "subject-" + hash,
      );
  ...
  describe("git-graph.version", () => {
    ...
    it("Should display the version information, and copy it to the clipboard", async () => {
      ...
      await waitForExpect(() => {
        expect(spyOnLog).toHaveBeenCalledWith(
          "Command Invoked: git-graph.version",
        );
        expect(vscode.window.showInformationMessage).toHaveBeenCalledWith(
          "Git Graph: 1.27.0\nVisual Studio Code: 1.51.0\nOS: X Y Z\nGit: 2.25.0",
          { modal: true },
  ...
  describe("git-graph.openFile", () => {
    ...
    it("Should open the provided file", async () => {
      ...
      // Assert
      expect(spyOnLog).toHaveBeenCalledWith(
        "Command Invoked: git-graph.openFile",
      );

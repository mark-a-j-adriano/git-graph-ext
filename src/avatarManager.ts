...
export class AvatarManager extends Disposable {
    ...
    if (typeof this.avatars[email] !== "undefined") {
      ...
      if (
        ...
        (this.avatars[email].identicon &&
          this.avatars[email].timestamp < t - 345600000)
      ) {
  ...
  public getAvatarImage(email: string) {
    return new Promise<string | null>((resolve) => {
      if (
        typeof this.avatars[email] !== "undefined" &&
        this.avatars[email].image !== null
      ) {
        fs.readFile(
          this.avatarStorageFolder + "/" + this.avatars[email].image,
          (err, data) => {
            resolve(
              err
                ? null
  ...
  private fetchFromGithub(
    avatarRequest: AvatarRequestItem,
    owner: string,
    repo: string,
  ) {
    let t = new Date().getTime();
    ...
    this.logger.log(
      "Requesting Avatar for " +
        maskEmail(avatarRequest.email) +
        " from GitHub",
    );
    ...
    const commitIndex =
      avatarRequest.commits.length < 5
        ...
        : Math.round(
            (4 - avatarRequest.attempts) *
              0.25 *
              (avatarRequest.commits.length - 1),
          );
        ...
        (res) => {
          ...
          res.on("end", async () => {
            if (res.headers["x-ratelimit-remaining"] === "0") {
              ...
              this.githubTimeout =
                parseInt(<string>res.headers["x-ratelimit-reset"]) * 1000;
              this.logger.log(
              ...
              if (commit.author && commit.author.avatar_url) {
                ...
                let img = await this.downloadAvatarImage(
                  ...
                  commit.author.avatar_url +
                    (commit.author.avatar_url.includes("?")
                      ? "&size=162"
                      : "?size=162"),
                );
                if (img !== null) {
                  ...
                  this.logger.log(
                    "Failed to download avatar from GitHub for " +
                      maskEmail(avatarRequest.email),
                  );
                }
                return;
        ...
        (res) => {
          ...
          res.on("end", async () => {
            if (res.headers["ratelimit-remaining"] === "0") {
              ...
              this.gitLabTimeout =
                parseInt(<string>res.headers["ratelimit-reset"]) * 1000;
              this.logger.log(
            ...
            if (res.statusCode === 200) {
              ...
              if (users.length > 0 && users[0].avatar_url) {
                ...
                if (img !== null) {
                  ...
                  this.logger.log(
                    "Failed to download avatar from GitLab for " +
                      maskEmail(avatarRequest.email),
                  );
                }
                return;
  ...
  private async fetchFromGravatar(avatarRequest: AvatarRequestItem) {
    ...
    if (img !== null) {
      ...
      this.logger.log(
        "No Avatar could be found for " + maskEmail(avatarRequest.email),
      );
    }
  }
  ...
  private saveAvatar(email: string, image: string, identicon: boolean) {
    ...
    this.emitAvatar(email).then(
      ...
      () =>
        this.logger.log(
          "Failed to Send Avatar for " +
            maskEmail(email) +
            " to the Git Graph View",
        ),
    );
  }
}
...
class AvatarRequestQueue {
  ...
  /**
   * Remove all queued avatar requests.
   */
  public clear() {
    this.queue = [];
  }
  ...
  public takeItem() {
    if (
      this.queue.length > 0 &&
      this.queue[0].checkAfter < new Date().getTime()
    )

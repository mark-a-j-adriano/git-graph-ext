...
class Config {
  ...
  get contextMenuActionsVisibility(): ContextMenuActionsVisibility {
    ...
    mapLegacyCreateArchiveVisibility(userConfig);
...
function mapLegacyCreateArchiveVisibility(user: { [key: string]: any }) {
  if (typeof user !== "object" || user === null) return;
  ["branch", "remoteBranch", "tag"].forEach((scope) => {
    if (
      typeof user[scope] === "object" &&
      user[scope] !== null &&
      typeof user[scope].createLocalBranch !== "boolean" &&
      typeof user[scope].createArchive === "boolean"
    ) {
      user[scope].createLocalBranch = user[scope].createArchive;
    }
  });
}

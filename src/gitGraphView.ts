...
export class GitGraphView extends Disposable {
  ...
  private async respondToMessage(msg: RequestMessage) {
    ...
    switch (msg.command) {
      ...
      case "checkoutBranch":
        errorInfos = [];
        if (
          msg.remoteBranch !== null &&
          this.shouldCreateBackupBranch(msg.remoteBranch)
        ) {
          errorInfos = errorInfos.concat(
            await this.dataSource.createBranch(
              msg.repo,
              this.getBackupBranchName(
                this.getBackupBranchNamePrefix(msg.remoteBranch),
              ),
              msg.remoteBranch,
              false,
              false,
        ...
        if (errorInfos.length === 0 || errorInfos[0] === null) {
          errorInfos.push(
        ...
        if (
          errorInfos[errorInfos.length - 1] === null &&
      ...
      case "cleanUntrackedFiles":
        ...
        break;
      case "commitDetails":
        const data: [GitCommitDetailsData, string | null] = await Promise.all([
  ...
  private shouldCreateBackupBranch(remoteBranch: string) {
    return this.getBackupBranchNamePrefix(remoteBranch) !== "main";
  }

  private getBackupBranchNamePrefix(remoteBranch: string) {
    const firstSlash = remoteBranch.indexOf("/");
    return firstSlash === -1
      ? remoteBranch
      : remoteBranch.substring(firstSlash + 1);
  }

  private getBackupBranchName(branchName: string) {
    const now = new Date();
    return (
      branchName +
      "-" +
    ...
    if (this.dataSource.isGitExecutableUnknown()) {
      ...
      body = `<body>
          ...
          <span id="worktreeStatusControl" class="unselectable"></span>
					...
					<label id="myCommitsControl"><input type="checkbox" id="myCommitsCheckbox" tabindex="-1"><span class="customCheckbox"></span>My Commits</label>

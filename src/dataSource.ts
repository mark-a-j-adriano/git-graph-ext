...
export class DataSource extends Disposable {
  			...
  			if (refData.head !== null && config.showUncommittedChanges) {
  				for (i = 0; i < commits.length; i++) {
  					if (refData.head === commits[i].hash) {
  						const numUncommittedChanges =
                await this.getUncommittedChanges(repo);
  ...
  public openExternalDirDiff(
  	...
  	return new Promise<ErrorInfo>((resolve) => {
  		if (this.gitExecutable === null) {
  			...
  			if (isGui) {
  				this.logger.log(
  					'External diff tool is being opened (' +
              args[args.length - 1] +
              ')'
  				);
  ...
  private getBranches(
  	...
  	return Promise.all([
  		this.spawnGit(args, repo, (stdout) => stdout),
  		this.getBranchReflogDates(repo).catch(
  			() =>
          ({}) as {
            [branchName: string]: { createdAt: number; updatedAt: number };
          }
  		)
  	]).then((results) => {
  		const branchReflogDates = results[1];
  		let branchData: GitBranchData = {
  			branches: [],
  			branchInfos: [],
  			head: null,
  			error: null
  		};
  		let lines = results[0].split(EOL_REGEX);
          ...
          (!showRemoteHeads && REMOTE_HEAD_BRANCH_REGEXP.test(name))
  			...
  			const branchInfo: GitBranchInfo = {
  				name: name,
  				createdAt:
            typeof branchReflogDates[name] !== 'undefined'
            	? branchReflogDates[name].createdAt
            	: null,
  				updatedAt:
            typeof branchReflogDates[name] !== 'undefined'
            	? branchReflogDates[name].updatedAt
            	: null,
  			...
  			if (lines[i][0] === '*') {
  				...
  				branchData.branchInfos.unshift(branchInfo);
  			} else {
  				branchData.branches.push(name);
  				branchData.branchInfos.push(branchInfo);
  			}
  		}
  		return branchData;
  	});
  ...
  private getAuthors(repo: string) {
  	return this.spawnGit(['shortlog', '-sne', '--all'], repo, (stdout) => {
  		let authorData: GitAuthorData = { authors: [], error: null };
  		let lines = stdout.split(EOL_REGEX);
  		for (let i = 0; i < lines.length - 1; i++) {
  			const match = lines[i].match(/^\s*(\d+)\s+(.+)\s+<([^<>]+)>$/);
  			if (match === null) continue;
  			authorData.authors.push({
  				commitCount: parseInt(match[1]),
  				name: match[2],
  				email: match[3],
  				displayName: match[2] + ' <' + match[3] + '>'
  			});
  		}
  		return authorData;
  	});
  ...
  private getBranchReflogDates(repo: string) {
  	return this.spawnGit(
  		['reflog', 'show', '--all', '--format=%gD' + GIT_LOG_SEPARATOR + '%ct'],
  		repo,
  		(stdout) => {
  			const branchReflogDates: {
          [branchName: string]: { createdAt: number; updatedAt: number };
        } = {};
  			...
  			for (let i = 0; i < lines.length - 1; i++) {
  				...
  				const createdAt = parseInt(line[1]);
  				if (Number.isNaN(createdAt)) continue;
  				const branchName = this.parseBranchNameFromReflogSelector(selector);
  				if (branchName === null) continue;

  				if (typeof branchReflogDates[branchName] === 'undefined') {
  					branchReflogDates[branchName] = {
  						createdAt: createdAt,
  						updatedAt: createdAt
  					};
  				} else {
  					if (createdAt < branchReflogDates[branchName].createdAt) {
  						branchReflogDates[branchName].createdAt = createdAt;
  					}
  					if (createdAt > branchReflogDates[branchName].updatedAt) {
  						branchReflogDates[branchName].updatedAt = createdAt;
  					}
  				}
  			}
  			return branchReflogDates;
  		}
  	);
  }

  private parseBranchNameFromReflogSelector(selector: string) {
  	const branchSelector = selector.replace(/@\{.*$/, '');
  	if (branchSelector.startsWith('refs/heads/')) {
  		return branchSelector.substring('refs/heads/'.length);
  	}
  	if (branchSelector.startsWith('refs/remotes/')) {
  		return 'remotes/' + branchSelector.substring('refs/remotes/'.length);
  	}
  	if (branchSelector === 'HEAD' || branchSelector.startsWith('refs/stash')) {
  		return null;
  	}
...
interface GitBranchData {
  branches: string[];
  branchInfos: GitBranchInfo[];
  head: string | null;
  error: ErrorInfo;
}

interface GitAuthorData {

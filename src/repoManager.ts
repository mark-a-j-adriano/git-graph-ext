...
import { BufferedQueue, BufferedQueueDiagnostics } from "./utils/bufferedQueue";
import { Disposable, toDisposable } from "./utils/disposable";
import { Event, EventEmitter } from "./utils/event";

export interface RepoManagerDiagnostics {
  readonly knownRepoCount: number;
  readonly ignoredRepoCount: number;
  readonly workspaceFolderWatcherCount: number;
  readonly watchedFolders: string[];
  readonly maxDepthOfRepoSearch: number;
  readonly onWatcherCreateQueue: BufferedQueueDiagnostics;
  readonly onWatcherChangeQueue: BufferedQueueDiagnostics;
  readonly checkRepoConfigQueue: BufferedQueueDiagnostics;
}
...
export class RepoManager extends Disposable {
  ...
  private readonly folderWatchers: {
    [workspace: string]: vscode.FileSystemWatcher;
  } = {};
  ...
  private workspaceSearchRequestId: number = 0;
  private activeWorkspaceSearchPromise: Promise<boolean> | null = null;
  private rerunWorkspaceSearch: boolean = false;
  ...
  constructor(
    ...
    this.configWatcher.onDidCreate(
      this.onConfigWatcherCreateOrChange.bind(this),
    );
  ...
  public registerRepo(path: string, loadRepo: boolean) {
    return new Promise<{ root: string | null; error: string | null }>(
      async (resolve) => {
        let root = await this.dataSource.repoRoot(path);
        if (root === null) {
          resolve({
            root: null,
            error: 'The folder "' + path + '" is not a Git repository.',
          });
          ...
          resolve({
            ...
            error:
              'The folder "' +
              path +
              '" is contained within the known repository "' +
              root +
              '".',
          });
        } else {
  ...
  /**
   * Get lightweight diagnostics for repository detection and watcher state.
   */
  public getDiagnostics(): RepoManagerDiagnostics {
    const watchedFolders = Object.keys(this.folderWatchers).sort();
    return {
      knownRepoCount: Object.keys(this.repos).length,
      ignoredRepoCount: this.ignoredRepos.length,
      workspaceFolderWatcherCount: watchedFolders.length,
      watchedFolders: watchedFolders,
      maxDepthOfRepoSearch: this.maxDepthOfRepoSearch,
      onWatcherCreateQueue: this.onWatcherCreateQueue.getDiagnostics(),
      onWatcherChangeQueue: this.onWatcherChangeQueue.getDiagnostics(),
      checkRepoConfigQueue: this.checkRepoConfigQueue.getDiagnostics(),
    };
  }
  ...
  private updateReposWorkspaceFolderIndex(repo: string | null = null) {
    ...
    const repoPaths =
      repo !== null && this.isKnownRepo(repo)
        ? [repo]
    ...
    for (let i = 0; i < repoPaths.length; i++) {
      ...
      if (rootIndex === -1) {
        ...
        rootIndex = rootsFolder.findIndex((root) =>
          repoPaths[i].startsWith(root),
        );
      }
      if (rootIndex === -1) {
        // Find a workspace folder that is contained within the repository
        const repoPathFolder = pathWithTrailingSlash(repoPaths[i]);
        rootIndex = rootsExact.findIndex((root) =>
          root.startsWith(repoPathFolder),
        );
      }
      workspaceFolderIndex =
        rootIndex > -1 ? workspaceFolders[rootIndex].index : null;
      if (
  ...
  public async searchWorkspaceForRepos() {
    this.workspaceSearchRequestId++;
    this.rerunWorkspaceSearch = true;

    if (this.activeWorkspaceSearchPromise !== null) {
      return this.activeWorkspaceSearchPromise;
    }

    this.activeWorkspaceSearchPromise = this.runWorkspaceRepoSearchLoop();
    try {
      return await this.activeWorkspaceSearchPromise;
    } finally {
      this.activeWorkspaceSearchPromise = null;
    }
  }

  /**
   * Run the workspace repo search loop, coalescing repeated requests and skipping stale traversals.
   */
  private async runWorkspaceRepoSearchLoop() {
    let changes = false;

    while (this.rerunWorkspaceSearch && !this.isDisposed()) {
      this.rerunWorkspaceSearch = false;
      const searchRequestId = this.workspaceSearchRequestId;
      const rootFolders = vscode.workspace.workspaceFolders;
      const startedAt = Date.now();
      let searchChanges = false;

      this.logger.log("Searching workspace for new repos ...");

      if (typeof rootFolders !== "undefined") {
        for (let i = 0; i < rootFolders.length; i++) {
          if (searchRequestId !== this.workspaceSearchRequestId) {
            break;
          }
      ...
      this.logger.logDuration(
        ...
        Date.now() - startedAt,
        250,
        (rootFolders?.length ?? 0) + " workspace folders",
      );

      if (searchRequestId !== this.workspaceSearchRequestId) {
        this.logger.log("Workspace repo search superseded by a newer request");
        continue;
      }
   ...
   * @returns TRUE => At least one repository was added, FALSE => No repositories were added.
   */
  private searchDirectoryForRepos(
    directory: string,
    maxDepth: number,
    ...
    return new Promise<boolean>((resolve) => {
      if (
        ...
        searchRequestId !== this.workspaceSearchRequestId
      ) {
        resolve(false);
        return;
      }
  ...
  private startWatchingFolder(path: string) {
    if (typeof this.folderWatchers[path] !== "undefined") return;
   ...
   */
  private async checkRepoForNewConfig(
    repo: string,
    isRepoNew: boolean = false,
  ) {
    try {
        ...
        if (validationError === null) {
          ...
          showErrorMessage(
            'The value for "' +
              ...
              getPathFromStr(
                path.join(repo, ".vscode", "vscode-git-graph.json"),
              ) +
              '" is invalid.',
          );
        }
...
export namespace ExternalRepoConfig {
  ...
  export type PullRequestConfig =
    | PullRequestConfigBuiltIn
    | PullRequestConfigCustom;
 ...
 */
function generateExternalConfigFile(
  state: GitRepoState,
): Readonly<ExternalRepoConfig.File> {
  ...
  if (state.onlyFollowFirstParent !== BooleanOverride.Default) {
    file.onlyFollowFirstParent =
      state.onlyFollowFirstParent === BooleanOverride.Enabled;
  }
  ...
  if (state.pullRequestConfig !== null) {
    ...
    file.pullRequestConfig = Object.assign({}, state.pullRequestConfig, {
      provider: provider,
    });
  }
  if (state.showRemoteBranchesV2 !== BooleanOverride.Default) {
    file.showRemoteBranches =
      state.showRemoteBranchesV2 === BooleanOverride.Enabled;
  }
...
function validateExternalConfigFile(file: Readonly<ExternalRepoConfig.File>) {
  ...
  if (
    ...
    (!Array.isArray(file.onRepoLoadShowSpecificBranches) ||
      file.onRepoLoadShowSpecificBranches.some(
  ...
  if (
    ...
    (typeof file.pullRequestConfig !== "object" ||
      ...
      (file.pullRequestConfig.provider !==
        ExternalRepoConfig.PullRequestProvider.Bitbucket &&
        (file.pullRequestConfig.provider !==
          ExternalRepoConfig.PullRequestProvider.Custom ||
        ...
        file.pullRequestConfig.provider !==
          ExternalRepoConfig.PullRequestProvider.GitHub &&
        file.pullRequestConfig.provider !==
          ExternalRepoConfig.PullRequestProvider.GitLab) ||
  ...
  if (
    typeof file.showStashes !== "undefined" &&
    typeof file.showStashes !== "boolean"
  ) {
    return "showStashes";
  }
  if (
    typeof file.showTags !== "undefined" &&
    typeof file.showTags !== "boolean"
  ) {
    return "showTags";
  }
  return null;
 ...
 */
function applyExternalConfigFile(
  file: Readonly<ExternalRepoConfig.File>,
  state: GitRepoState,
) {
  ...
  if (typeof file.includeCommitsMentionedByReflogs !== "undefined") {
    state.includeCommitsMentionedByReflogs =
      file.includeCommitsMentionedByReflogs
  ...
  if (typeof file.showStashes !== "undefined") {
    state.showStashes = file.showStashes
      ? BooleanOverride.Enabled
      : BooleanOverride.Disabled;
  }
  if (typeof file.showTags !== "undefined") {
    state.showTags = file.showTags
      ? BooleanOverride.Enabled
      : BooleanOverride.Disabled;
  }
}

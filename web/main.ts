const BRANCH_AGE_FILTER_OPTIONS: ReadonlyArray<{
  readonly value: string;
  readonly name: string;
  readonly days: number | null;
}> = [
  { value: "", name: "Any time", days: null },
  { value: "1d", name: "1 day", days: 1 },
  { value: "2d", name: "2 days", days: 2 },
  { value: "3d", name: "3 days", days: 3 },
  { value: "4d", name: "4 days", days: 4 },
  { value: "5d", name: "5 days", days: 5 },
  { value: "6d", name: "6 days", days: 6 },
  { value: "1w", name: "1 week", days: 7 },
  { value: "2w", name: "2 weeks", days: 14 },
  { value: "3w", name: "3 weeks", days: 21 },
  { value: "1m", name: "1 month", days: 30 },
  { value: "2m", name: "2 months", days: 60 },
  { value: "3m", name: "3 months", days: 90 },
  { value: "6m", name: "6 months", days: 180 },
  { value: "9m", name: "9 months", days: 270 },
  { value: "1y", name: "1 year", days: 365 },
];

const WORKTREE_COLOR_THEME_OPTIONS: ReadonlyArray<{
  readonly name: string;
  readonly value: string;
}> = [
  { name: "None", value: "" },
  { name: "Blue", value: "peacock-blue" },
  { name: "Green", value: "peacock-green" },
  { name: "Purple", value: "peacock-purple" },
  { name: "Orange", value: "peacock-orange" },
  { name: "Teal", value: "peacock-teal" },
];

class GitGraphView {
  private gitRepos: GG.GitRepoSet;
  private gitBranches: ReadonlyArray<string> = [];
  private gitBranchInfos: ReadonlyArray<GG.GitBranchInfo> = [];
  private gitWorktrees: ReadonlyArray<GG.GitWorktreeInfo> = [];
  ...
  private branchCreationDateFilter = "";
  private branchUpdatedDateFilter = "";
  ...
  private readonly worktreeStatusElem: HTMLElement;
  private readonly branchCreationDateDropdownElem: HTMLSelectElement;
  private readonly branchUpdatedDateDropdownElem: HTMLSelectElement;
  ...
  constructor(viewElem: HTMLElement, prevState: WebViewState | null) {
    ...
    this.worktreeStatusElem = document.getElementById("worktreeStatusControl")!;
    ...
    this.branchCreationDateDropdownElem = <HTMLSelectElement>(
      document.getElementById("branchCreationDateDropdown")!
    );
    this.branchUpdatedDateDropdownElem = <HTMLSelectElement>(
      document.getElementById("branchUpdatedDateDropdown")!
    );
    this.populateBranchAgeFilterDropdown(this.branchCreationDateDropdownElem);
    this.populateBranchAgeFilterDropdown(this.branchUpdatedDateDropdownElem);
    this.branchCreationDateDropdownElem.addEventListener("change", () => {
      this.branchCreationDateFilter = this.normaliseBranchAgeFilter(
        this.branchCreationDateDropdownElem.value,
      );
      this.branchCreationDateDropdownElem.value = this.branchCreationDateFilter;
      this.applyBranchDateFilters();
    });
    this.branchUpdatedDateDropdownElem.addEventListener("change", () => {
      this.branchUpdatedDateFilter = this.normaliseBranchAgeFilter(
        this.branchUpdatedDateDropdownElem.value,
      );
      this.branchUpdatedDateDropdownElem.value = this.branchUpdatedDateFilter;
      this.applyBranchDateFilters();
    });

    this.myCommitsElem = <HTMLInputElement>(
      document.getElementById("myCommitsCheckbox")!
    );
    ...
    this.observeWorktreeStatusEvents();
    ...
    if (
      ...
      this.branchCreationDateFilter =
        typeof prevState.branchCreationDateFilter === "string"
          ? this.normaliseBranchAgeFilter(prevState.branchCreationDateFilter)
          : "";
      this.branchUpdatedDateFilter =
        typeof prevState.branchUpdatedDateFilter === "string"
          ? this.normaliseBranchAgeFilter(prevState.branchUpdatedDateFilter)
          : "";
      ...
      this.loadRepoInfo(
        ...
        Array.isArray(prevState.gitBranchInfos) ? prevState.gitBranchInfos : [],
        Array.isArray(prevState.gitWorktrees) ? prevState.gitWorktrees : [],
  ...
  private loadRepoInfo(
    ...
    branchInfos: ReadonlyArray<GG.GitBranchInfo>,
    worktrees: ReadonlyArray<GG.GitWorktreeInfo>,
    ...
    this.gitBranchInfos =
      branchInfos.length > 0
        ...
        : branchOptions.map((branch) => ({
            ...
            createdAt: null,
            updatedAt: null,
            isRemote: branch.indexOf("remotes/") === 0,
          }));
    this.gitWorktrees = worktrees;
    this.gitBranchHead = branchHead;
    this.gitRemotes = remotes;

    // Update the state of the fetch button
    this.renderFetchButton();
    this.renderWorktreeStatus();
    ...
    if (
      ...
      this.currentBranches = this.currentBranches.filter(
        (branch) =>
          visibleBranches.includes(branch) || globPatterns.includes(branch),
      );
    }
  ...
  public processLoadConfig(msg: GG.ResponseLoadConfig) {
    ...
    if (
      ...
      previousAuthorFilter !== this.getAuthorFilter()
    ) {
      this.clearCommits();
      this.requestLoadRepoInfoAndCommits(true, true);
    }
  ...
  private getVisibleBranchInfos(): ReadonlyArray<GG.GitBranchInfo> {
    const maxCreatedAgeInDays = this.parseBranchAgeFilter(
      this.branchCreationDateFilter,
    );
    const maxUpdatedAgeInDays = this.parseBranchAgeFilter(
      this.branchUpdatedDateFilter,
    );
    const nowInSeconds = Math.floor(Date.now() / 1000);
    const mainBranchInfos = this.gitBranchInfos.filter((branchInfo) =>
      this.isMainBranch(branchInfo.name),
    );

    const visibleBranchInfos = this.gitBranchInfos.filter((branchInfo) => {
      if (
        maxCreatedAgeInDays !== null &&
        (branchInfo.createdAt === null ||
          branchInfo.createdAt < nowInSeconds - maxCreatedAgeInDays * 86400)
      ) {
        return false;
      }

      if (
        maxUpdatedAgeInDays !== null &&
        (branchInfo.updatedAt === null ||
          branchInfo.updatedAt < nowInSeconds - maxUpdatedAgeInDays * 86400)
      ) {
        return false;
      }
    ...
    for (let i = mainBranchInfos.length - 1; i >= 0; i--) {
      if (!visibleBranchInfos.includes(mainBranchInfos[i])) {
        visibleBranchInfos.unshift(mainBranchInfos[i]);
      }
    }

    return visibleBranchInfos;
  }

  private isMainBranch(branchName: string) {
    return branchName === "main" || branchName.endsWith("/main");
  }

  private getVisibleBranches(): ReadonlyArray<string> {
    return this.getVisibleBranchInfos().map((branchInfo) => branchInfo.name);
  }

  private getBranchWorktrees(branchName: string) {
    return this.gitWorktrees.filter(
      (worktree) => worktree.branch === branchName,
    );
  }

  private getCurrentRepoWorktree() {
    for (let i = 0; i < this.gitWorktrees.length; i++) {
      if (this.gitWorktrees[i].isCurrent) return this.gitWorktrees[i];
    }
    return null;
  }

  private getLinkedWorktrees(branchName?: string) {
    return this.gitWorktrees.filter(
      (worktree) =>
        !worktree.isCurrent &&
        (typeof branchName === "undefined" || worktree.branch === branchName),
    );
  }

  private getWorktreeLabel(worktree: GG.GitWorktreeInfo) {
    return (
      (worktree.branch !== null ? worktree.branch : "detached") +
      " - " +
      worktree.path +
  ...
  private selectWorktree(
    worktrees: ReadonlyArray<GG.GitWorktreeInfo>,
    message: string,
    actionName: string,
    actioned: (worktree: GG.GitWorktreeInfo) => void,
    target: DialogTarget | null,
  ) {
    if (worktrees.length === 0) return;
    if (worktrees.length === 1) {
      actioned(worktrees[0]);
      return;
    }
    ...
    dialog.showSelect(
      ...
      worktrees.map((worktree) => ({
        name: this.getWorktreeLabel(worktree),
        value: worktree.path,
      })),
      actionName,
      (worktreePath) => {
        ...
        if (typeof selectedWorktree !== "undefined") {
          actioned(selectedWorktree);
        }
      },
      target,
    );
  ...
  private getWorktreeRefBadgeHtml(branchName: string) {
    const worktrees = this.getBranchWorktrees(branchName);
    if (worktrees.length === 0) return "";

    const hasCurrentWorktree = worktrees.some((worktree) => worktree.isCurrent);
    const badgeText =
      "WT" +
      worktrees
        .map((worktree) => this.getWorktreeIndex(worktree))
        .filter((worktreeIndex) => worktreeIndex !== null)
        .join("/");
    const title = worktrees
      .map((worktree) => this.getWorktreeLabel(worktree))
      .join("\n");
    return (
      '<span class="gitRefWorktreeBadge' +
      (hasCurrentWorktree ? " current" : "") +
      '" title="' +
      escapeHtml(title) +
      '">' +
  ...
  private getWorktreeIndex(worktree: GG.GitWorktreeInfo) {
    const index = this.gitWorktrees.findIndex(
      (candidateWorktree) => candidateWorktree.path === worktree.path,
    );
    return index === -1 ? null : index + 1;
  }

  private renderWorktreeStatus() {
    if (this.gitWorktrees.length <= 1) {
      this.worktreeStatusElem.textContent = "";
      this.worktreeStatusElem.style.display = "none";
      this.worktreeStatusElem.title = "";
      return;
    }

    const currentWorktree = this.getCurrentRepoWorktree();
    const prunableCount = this.gitWorktrees.filter(
      (worktree) => worktree.prunable,
    ).length;
    this.worktreeStatusElem.textContent =
      "Worktrees: " +
      this.gitWorktrees.length +
      "  Current: " +
      (currentWorktree !== null && currentWorktree.branch !== null
        ? currentWorktree.branch
        : "detached");
    if (prunableCount > 0) {
      this.worktreeStatusElem.textContent += "  Stale: " + prunableCount;
    }
    this.worktreeStatusElem.title =
      currentWorktree !== null ? currentWorktree.path : "Linked worktrees";
    this.worktreeStatusElem.style.display = "inline-block";
  }

  public getBranchOptions(
    includeShowAll?: boolean,
  ): ReadonlyArray<{ name: string; value: string; hint?: string }> {
    const options: { name: string; value: string; hint?: string }[] = [];
    if (includeShowAll) {
    ...
    const visibleBranchInfos = this.getVisibleBranchInfos();
    for (let i = 0; i < visibleBranchInfos.length; i++) {
      const createdAt = visibleBranchInfos[i].createdAt;
      const updatedAt = visibleBranchInfos[i].updatedAt;
      const worktrees = this.getBranchWorktrees(visibleBranchInfos[i].name);
      const hintParts: string[] = [];
      if (createdAt !== null)
        hintParts.push("C: " + formatShortDate(createdAt).formatted);
      if (updatedAt !== null)
        hintParts.push("U: " + formatShortDate(updatedAt).formatted);
      if (worktrees.length > 0) {
        hintParts.push(
          worktrees.some((worktree) => worktree.isCurrent)
            ? "WT: " + worktrees.length + " here"
            : "WT: " + worktrees.length,
        );
      }
      options.push({
        name:
          visibleBranchInfos[i].name.indexOf("remotes/") === 0
            ? visibleBranchInfos[i].name.substring(8)
            : visibleBranchInfos[i].name,
        value: visibleBranchInfos[i].name,
        hint:
          hintParts.length > 0
            ? hintParts.join("  ")
            : visibleBranchInfos[i].isRemote
              ? "remote"
              : "unknown",
      });
    }
  ...
  private populateBranchAgeFilterDropdown(selectElem: HTMLSelectElement) {
    for (let i = 0; i < BRANCH_AGE_FILTER_OPTIONS.length; i++) {
      const optionElem = document.createElement("option");
      optionElem.value = BRANCH_AGE_FILTER_OPTIONS[i].value;
      optionElem.textContent = BRANCH_AGE_FILTER_OPTIONS[i].name;
      selectElem.appendChild(optionElem);
    }
  }

  private normaliseBranchAgeFilter(value: string): string {
    const trimmed = value.trim();
    if (trimmed === "") return "";

    if (BRANCH_AGE_FILTER_OPTIONS.some((option) => option.value === trimmed)) {
      return trimmed;
    }
  ...
  private parseBranchAgeFilter(value: string): number | null {
    for (let i = 0; i < BRANCH_AGE_FILTER_OPTIONS.length; i++) {
      if (BRANCH_AGE_FILTER_OPTIONS[i].value === value) {
        return BRANCH_AGE_FILTER_OPTIONS[i].days;
      }
    }
  ...
  private areBranchDateFiltersActive(): boolean {
    return (
      this.branchCreationDateFilter !== "" ||
      this.branchUpdatedDateFilter !== ""
    );
  ...
  private applyBranchDateFilters() {
    this.maxCommits = this.config.initialLoadCommits;
  ...
  private getAuthorFilter() {
    return this.myCommitsOnly ? this.getMyCommitsAuthorFilter() : null;
  }
  ...
  private renderMyCommitsControl() {
    const myCommitsControl = <HTMLLabelElement>(
      document.getElementById("myCommitsControl")!
    );
    const authorFilter = this.getMyCommitsAuthorFilter();
    this.myCommitsElem.checked = this.myCommitsOnly;
    this.myCommitsElem.disabled =
      this.gitConfig === null || authorFilter === null;
    myCommitsControl.title =
      this.gitConfig === null
  ...
  private requestLoadCommits() {
    ...
    const branches =
      ...
      (this.currentBranches.length === 1 &&
        this.currentBranches[0] === SHOW_ALL_BRANCHES)
        ? !this.areBranchDateFiltersActive()
          ? null
          : visibleBranches
        : this.currentBranches;
    sendMessage({
  ...
  public saveState() {
    ...
    VSCODE_API.setState({
      ...
      gitBranchInfos: this.gitBranchInfos,
      ...
      branchCreationDateFilter: this.branchCreationDateFilter,
      branchUpdatedDateFilter: this.branchUpdatedDateFilter,
  ...
  private renderTable() {
    ...
    for (let i = 0; i < this.commits.length; i++) {
      ...
      for (j = 0; j < branchLabels.heads.length; j++) {
        ...
        refActive = branchName === this.gitBranchHead;
        refHtml =
        ...
        refHtml += this.getWorktreeRefBadgeHtml(branchName);
        refHtml += "</span>";
  ...
  private getBranchContextMenuActions(
    ...
    const linkedWorktrees = this.getBranchWorktrees(refName).filter(
      ...
      [
        {
          ...
          visible: visibility.createWorktree && this.gitBranchHead !== refName,
          onClick: () => this.createWorktreeAction(refName, target),
        },
        {
          title: "Open Linked Worktree in New Window" + ELLIPSIS,
          visible: visibility.createWorktree && linkedWorktrees.length > 0,
          onClick: () => this.openBranchWorktreeAction(refName, target),
        },
        {
          title: "Reveal Linked Worktree in File Explorer" + ELLIPSIS,
          visible: visibility.createWorktree && linkedWorktrees.length > 0,
          onClick: () => this.revealBranchWorktreeAction(refName, target),
        },
        {
          title: "Remove Linked Worktree" + ELLIPSIS,
          visible: visibility.createWorktree && linkedWorktrees.length > 0,
          onClick: () => this.removeBranchWorktreeAction(refName, target),
        },
      ],
      [
        {
      ...
      [
        {
          title: "Create Worktree" + ELLIPSIS,
          visible:
            visibility.createWorktree && remote !== "" && branchName !== "HEAD",
          onClick: () =>
            this.createRemoteBranchWorktreeAction(branchName, refName, target),
        },
      ],
      [
        this.getViewIssueAction(refName, visibility.viewIssue, target),
  ...
  private createWorktreeAction(
    ...
    const defaultWorktreePath = this.getDefaultWorktreePath(branchName);
    dialog.showForm(
      "Create a worktree for branch <b><i>" +
        escapeHtml(branchName) +
        "</i></b>:",
      [
        {
          type: DialogInputType.Text,
          name: "Path",
          default: defaultWorktreePath,
          placeholder: null,
        },
        {
          type: DialogInputType.Checkbox,
          name: "Open in a new VS Code window",
          value: false,
        },
        {
          type: DialogInputType.Select,
          name: "Theme",
          options: WORKTREE_COLOR_THEME_OPTIONS,
          default: "",
        },
      ],
      "Create Worktree",
        ...
        runAction(
          {
            ...
            repo: this.currentRepo,
            branchName: branchName,
            worktreePath: <string>values[0],
            openInNewWindow: <boolean>values[1],
            colorTheme: values[2] === "" ? null : <string>values[2],
            startPoint: null,
          },
          "Creating Worktree",
        );
  ...
  private createRemoteBranchWorktreeAction(
    branchName: string,
    remoteBranch: string,
    target: DialogTarget & RefTarget,
  ) {
    dialog.showForm(
      "Create a worktree from remote branch <b><i>" +
        escapeHtml(remoteBranch) +
        "</i></b>:",
      [
        {
          type: DialogInputType.TextRef,
        ...
        },
        {
          type: DialogInputType.Text,
          name: "Path",
          default: this.getDefaultWorktreePath(branchName),
          placeholder: null,
        },
        {
          type: DialogInputType.Checkbox,
      ...
      "Create Worktree",
      (values) => {
        const worktreeBranchName = <string>values[0];
        runAction(
          {
            command: "createWorktree",
            repo: this.currentRepo,
            branchName: worktreeBranchName,
            worktreePath: <string>values[1],
            openInNewWindow: <boolean>values[2],
            colorTheme: values[3] === "" ? null : <string>values[3],
  ...
  private getDefaultWorktreePath(branchName: string) {
    const normalisedBranchName = this.getWorktreeDirectoryName(branchName);
    const lastSlashIndex = this.currentRepo.lastIndexOf("/");
    const parentPath =
      lastSlashIndex >= 0
        ? this.currentRepo.substring(0, lastSlashIndex)
        : this.currentRepo;
    return (
      parentPath +
      "/" +
  ...
  private getWorktreeDirectoryName(branchName: string) {
    const normalisedBranchName = branchName
      .replace(/^remotes\/[^/]+\//, "")
      .replace(/^remotes\//, "")
      .replace(/\\/g, "/")
      .toLowerCase()
      ...
      .replace(/^-+|-+$/g, "")
      .replace(/-+/g, "-");
    return normalisedBranchName !== "" ? normalisedBranchName : "worktree";
  }

  private openBranchWorktreeAction(
    branchName: string,
    target: DialogTarget & RefTarget,
  ) {
    const worktrees = this.getLinkedWorktrees(branchName);
    this.selectWorktree(
      worktrees,
      "Open a linked worktree for branch <b><i>" +
        escapeHtml(branchName) +
        "</i></b>:",
      "Open Worktree",
      (worktree) => {
  ...
  private revealBranchWorktreeAction(
    branchName: string,
    target: DialogTarget & RefTarget,
  ) {
    const worktrees = this.getLinkedWorktrees(branchName);
    this.selectWorktree(
      worktrees,
      "Reveal a linked worktree for branch <b><i>" +
        escapeHtml(branchName) +
        "</i></b> in the file explorer:",
      "Reveal Worktree",
      (worktree) => {
        runAction(
          {
  ...
  private removeBranchWorktreeAction(
    branchName: string,
    target: DialogTarget & RefTarget,
  ) {
    const worktrees = this.getLinkedWorktrees(branchName);
    this.selectWorktree(
      worktrees,
      "Remove a linked worktree for branch <b><i>" +
        escapeHtml(branchName) +
        "</i></b>:",
      "Choose Worktree",
      (worktree) => {
        dialog.showForm(
          "Are you sure you want to remove the worktree <b><i>" +
          ...
          [
            {
              ...
              name: "Force Remove",
              value: false,
              info: "Force removal when Git detects uncommitted changes or missing metadata.",
            },
          ],
          "Remove Worktree",
          (values) => {
  ...
  private getWorktreeStatusContextMenuActions(): ContextMenuActions {
    const linkedWorktrees = this.getLinkedWorktrees();
    const prunableCount = this.gitWorktrees.filter(
      (worktree) => worktree.prunable,
    ).length;
    return [
      [
        {
          title: "Open Linked Worktree in New Window" + ELLIPSIS,
          visible: linkedWorktrees.length > 0,
          onClick: () =>
            this.selectWorktree(
              linkedWorktrees,
              "Open a linked worktree in a new window:",
              "Open Worktree",
              (worktree) => {
                runAction(
                  {
        ...
        {
          title: "Reveal Worktree in File Explorer" + ELLIPSIS,
          visible: this.gitWorktrees.length > 0,
          onClick: () =>
            this.selectWorktree(
              this.gitWorktrees,
              "Reveal a worktree in the file explorer:",
              "Reveal Worktree",
              (worktree) => {
                runAction(
                  {
      ...
      [
        {
          ...
          visible: prunableCount > 0,
          onClick: () => {
            dialog.showConfirmation(
              "Prune stale worktree metadata for this repository?",
              "Yes, prune",
              () => {
                runAction(
                  {
  ...
  private observeWorktreeStatusEvents() {
    const showWorktreeStatusMenu = (e: MouseEvent) => {
      if (this.gitWorktrees.length === 0) return;
      handledEvent(e);
      contextMenu.show(
        this.getWorktreeStatusContextMenuActions(),
        false,
        null,
        e,
        this.viewElem,
    ...
    this.worktreeStatusElem.addEventListener("click", showWorktreeStatusMenu);
    this.worktreeStatusElem.addEventListener(
      "contextmenu",
      showWorktreeStatusMenu,
    );
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    switch (msg.command) {
      ...
      case "createBranch":
        ...
        break;
      case "createWorktree":
        finishOrDisplayError(msg.error, "Unable to Create Worktree", true);
        break;
      case "openWorktree":
        finishOrDisplayError(msg.error, "Unable to Open Worktree", true);
        break;
      case "pruneWorktrees":
        refreshOrDisplayError(msg.error, "Unable to Prune Worktrees");
        break;
      ...
      case "renameBranch":
        ...
        break;
      case "removeWorktree":
        refreshOrDisplayError(msg.error, "Unable to Remove Worktree");
        break;
      case "revealWorktree":
        finishOrDisplayError(msg.error, "Unable to Reveal Worktree", true);
        break;
